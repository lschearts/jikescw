...
function wppay_plugin_action_link($actions, $plugin_file, $plugin_data){
	if( strpos($plugin_file, 'wp-wppay') !== false && is_plugin_active($plugin_file) ){
		$myactions = array(
			'option' => "<a href=\"" . WPAY_ADMIN_URL . "options-general.php?page=wppay.functions.php\">设置</a>"
		);
...
function wppay_scripts(){
	...
	$this_js = (_hui('is_mianqian','0')) ? 'mqy-pay' : 'qy-pay' ;
...
function alipay_callback(){
	...
	$activity_Time=strtotime(_hui('activity_end_time'));
	if ($order_type == 1) {
		// 文章价格
		$price = get_post_meta($post_id,'wppay_price',true);
	}else if ($order_type == 2) {
		// 月会员价格
		$price = _hui('vip_price_31');
	}else if ($order_type == 3) {
		// 年费会员价格
		$price = _hui('vip_price_365');
	}else if ($order_type == 4) {
		// 终身会员价格
		if(($activity_Time>=time()) && _hui('activity_end_open') && !empty(_hui('activity_price'))){
			//活动期间减价的价格
		$price =intval(_hui('activity_price'));
	}else{
		$price = _hui('vip_price_3600');
	}
...
function wppay_content_show($content){
	...
	if($price && $type == '1'){
		...
		if($wppay->is_paid()){
			...
			if(is_singular()){
				$paytype = wppay_get_setting('pay-type');
				if (_hui('no_loginpay') == 0 && !is_user_logged_in()) {
					...
					$wpayc = '<div class="erphp-wppay all"><p>此文章为付费文章'.$vip_infotext.'</p><a href="javascript:;" id="pay-loader" class="wppay-loader" data-post="'.$post->ID.'" data-paytypes="'.$paytype.'" ><i class="iconfont">&#xe70c;</i> 支付'.$price.'元查看</a></div>';
				}
