...
class WPAY
{
	...
	public function add($out_trade_no, $price ,$order_type,$pay_type)
	{
		...
		global $wpdb, $wppay_table_name;
		$sql = $wpdb->insert($wppay_table_name, array('order_num' => $out_trade_no, 'order_type' => $order_type, 'pay_type' => $pay_type, 'post_id' => $this->post_id, 'order_price' => $price,'user_id' => $this->user_id,'create_time' => time()), array('%s', '%d', '%d','%d', '%s', '%d','%s', '%s'));
		if ($sql) {
			return true;
		}
		return false;
