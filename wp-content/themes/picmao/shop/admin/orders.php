...
//按条件按钮排
if($_GET['page']=='wppay_orders_page'  ){
	    	if($_GET['selled']=='yesed'){
	    	$list = $wpdb->get_results("SELECT * FROM $wppay_table_name WHERE order_type =1 AND status=1 ORDER BY create_time DESC limit $offset,$perpage");
	    	if($_GET['author']=='named'){
	    		$list = $wpdb->get_results("SELECT * FROM $wppay_table_name WHERE order_type =1 AND status=1 ORDER BY author_order DESC ,create_time DESC limit $offset,$perpage");
	...
	<input type="submit" id="search-submit" class="button" value="搜索订单"></p>
	    ...
	    <div class="alignleft actions">
	        ...
	        <input class="button" id="post-query-submit" name="filter_action" type="submit" value="筛选">
      		...
      		<a class="button" type="submit" href="<?php echo admin_url('admin.php?page=wppay_orders_page'); ?>" >所有订单
	        </a>
	        <a class="button" type="submit" href="<?php echo admin_url('admin.php?page=wppay_orders_page&selled=yesed'); ?>" >已支付
	        </a>
	        <a class="button" type="submit" href="<?php echo admin_url('admin.php?page=wppay_orders_page&selled=yesed&author=named'); ?>" >已支付的作者统计
	        </a>
	    </div>
		...
		if($list) {
			foreach($list as $value){
				...
				echo '<td>'.date('Y-m-d h:i:s',$value->create_time).'</td>';
              	$author_name=$r_author_name=get_the_author_meta('display_name',get_post($value->post_id)->post_author);
				$e_author_name='<p style="color:red;">'.$r_author_name.'</p>';
              	echo '<td>'.($value->author_order==$author_name?$r_author_name:$e_author_name).'</td>';//作者昵称
